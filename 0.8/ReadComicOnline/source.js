...
var _Sources = (() => {
  ...
  // src/ReadComicOnline/ReadComicOnline.ts
  var ReadComicOnline_exports = {};
  __export(ReadComicOnline_exports, {
    ReadComicOnline: () => ReadComicOnline,
    ReadComicOnlineInfo: () => ReadComicOnlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/ReadComicOnline/ReadComicOnlineParser.ts
  ...
  var parseMangaDetails = ($, mangaId) => {
    ...
    const titles = [decodeHTML($("a.bigChar").text().trim())];
    ...
    const artist = $("a", $("span:contains(Artist)", contentSection).parent()).text().trim() ?? "";
    const description = decodeHTML($("span:contains(Summary)", contentSection).parent().next().text().trim() ?? "");
    let hentai = false;
    const arrayTags = [];
  ...
  var DISABLE_JS_SCRIPT = trimIndent(`
    const handler = {
...
`);
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestSection = App.createHomeSection({ id: "latest_comic", title: "Latest Updated Comics", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal });
    const newSection = App.createHomeSection({ id: "new_comic", title: "New Comics", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal });
    const popularSection = App.createHomeSection({ id: "popular_comic", title: "Most Popular Comics", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal });
    const TopDaySection = App.createHomeSection({ id: "top_day_comic", title: "Top Day Comics", containsMoreItems: false, type: import_types.HomeSectionType.singleRowNormal });
    const TopWeekSection = App.createHomeSection({ id: "top_week_comic", title: "Top Week Comics", containsMoreItems: false, type: import_types.HomeSectionType.singleRowNormal });
    const TopMonthSection = App.createHomeSection({ id: "top_month_comic", title: "Top Month Comics", containsMoreItems: false, type: import_types.HomeSectionType.singleRowNormal });
    const latestSection_Array = [];
    for (const comic of $("a", $("div.items", "div.bigBarContainer")).toArray()) {
      let image = $("img", comic).first().attr("src") ?? "";
      if (image == "") image = $("img", comic).first().attr("srctemp") ?? "";
      image = image.startsWith("/") ? RCO_DOMAIN + image : image;
      const title = $(comic).contents().not("span").text().trim() ?? "";
      const id = $(comic).attr("href")?.replace(/comic\//i, "") ?? "";
      const subtitle = $(comic).attr("title") ?? "";
    ...
    for (const comic of $("div", "div#tab-newest").toArray()) {
      let image = $("img", comic).first().attr("src") ?? "https://i.imgur.com/GYUxEX8.png";
      image = image.startsWith("/") ? RCO_DOMAIN + image : image;
      const title = $("a.title", comic).last().text().trim() ?? "";
      const id = $("a", comic).attr("href")?.replace(/comic\//i, "") ?? "";
  ...
  var parseViewMore = ($) => {
    ...
    for (const item of $(".list-comic > .item > a:first-child").toArray()) {
      const title = $(item).first().text().trim() ?? "";
      const id = $(item).attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      let image = $("img", item).attr("src") ?? "https://i.imgur.com/GYUxEX8.png";
      image = image.startsWith("/") ? RCO_DOMAIN + image : image;
  ...
  var parseSearch = ($) => {
    ...
    if (directMatch.toLocaleLowerCase() == "cover") {
      ...
      const id = ($("a"), $(".bigChar").attr("href")?.replace(/comic\//i, "")) ?? "";
      let image = $("img", $(".rightBox")).attr("src") ?? "https://i.imgur.com/GYUxEX8.png";
      image = image.startsWith("/") ? RCO_DOMAIN + image : image;
  ...
  // src/ReadComicOnline/ReadComicOnline.ts
  var RCO_DOMAIN2 = "https://readcomiconline.li";
  var ReadComicOnlineInfo = {
    version: "2.0.3",
    ...
    intents: import_types2.SourceIntents.HOMEPAGE_SECTIONS | import_types2.SourceIntents.MANGA_CHAPTERS | import_types2.SourceIntents.CLOUDFLARE_BYPASS_REQUIRED,
    contentRating: import_types2.ContentRating.MATURE,
    websiteBaseURL: RCO_DOMAIN2,
    sourceTags: [
      {
        text: "Cloudflare",
  ...
  var ReadComicOnline = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RCO_DOMAIN2}/Comic/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RCO_DOMAIN2}/Comic/`,
        method: "GET",
        param: mangaId
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RCO_DOMAIN2}/Comic/${mangaId}/${chapterId}`,
        method: "GET",
      ...
      return await parseChapterDetails(response.data, rguardUrl, mangaId, chapterId, this);
    }
    async getSearchTags() {
      const request = App.createRequest({
        url: `${RCO_DOMAIN2}/ComicList`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
...
Please go to Settings > Sources > ${ReadComicOnlineInfo.name} and press Cloudflare Bypass`);
  ...
  return __toCommonJS(ReadComicOnline_exports);
})();
