...
var _Sources = (() => {
  ...
  // src/VoyceME/VoyceME.ts
  var VoyceME_exports = {};
  __export(VoyceME_exports, {
    VoyceME: () => VoyceME,
    VoyceMEInfo: () => VoyceMEInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/VoyceME/VoyceMEGraphQL.ts
  var HomePageQuery = (page, popularPerPage) => ({
  ...
  var SearchQuery = (query, page, popularPerPage) => {
    const Genres = [];
    const Types = [];
    const Status = [];
    let Category = false;
  ...
  var ChapterDetailsQuery = (chapterId) => ({
...
${"".padEnd(offset)}${"^".repeat(len)}`;
  ...
  var Parser2 = class {
    ...
    parseMangaDetails(VoyceD, mangaId) {
      ...
      for (const obj of details?.genres ?? []) {
        ...
        const title2 = obj?.genre.title.trim() ?? "";
    ...
    parseChapters(VoyceD) {
      ...
      for (const obj of data?.chapters ?? []) {
        const id = obj.id ?? "";
        const name2 = obj.title ?? "No Chpater Name";
        const release_date = obj.created_at;
        const chapNum = Number(/\d+/.exec(name2)?.pop()?.replace(/-/g, "."));
        if (!id) continue;
  ...
  // src/VoyceME/VoyceME.ts
  var VoyceME_Base = "http://voyce.me";
  var VoyceMEInfo = {
    author: "xOnlyFadi",
  ...
  var VoyceME = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      metadata = manga.length == this.popularPerPage ? { page: page + 1 } : void 0;
    ...
    async getMangaDetails(mangaId) {
      ...
      if (data.data.series == void 0) throw new Error(`Failed to parse manga property from data object mangaId: ${mangaId}`);
    ...
    async getChapters(mangaId) {
      ...
      if (data.data?.series == void 0) throw new Error(`Failed to parse manga property from data object mangaId: ${mangaId}`);
      if (data.data?.series[0]?.chapters == void 0) throw new Error(`Failed to parse chapters property from manga object mangaId: ${mangaId}`);
  ...
  return __toCommonJS(VoyceME_exports);
})();
