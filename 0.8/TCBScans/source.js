...
var _Sources = (() => {
  ...
  var TCBScans_exports = {};
  __export(TCBScans_exports, {
    TCBScans: () => TCBScans,
    TCBScansInfo: () => TCBScansInfo
  });
  ...
  // src/TCBScans/TCBScansParser.ts
  var Parser3 = class {
    constructor() {
      this.chapterTitleRegex = /Chapter ([\d.]+)/i;
    }
    async parseHomeSections($2, sectionCallback) {
      const section1 = App.createHomeSection({ id: "1", title: "Projects", containsMoreItems: false, type: "singleRowNormal" });
      const projects = [];
      const arrManga = $2(".bg-card.border.border-border.rounded.p-3.mb-3").toArray();
      for (const obj of arrManga) {
        const id = $2("a.mb-3.text-white.text-lg.font-bold", obj).attr("href")?.replace(/\/mangas\//gi, "") ?? "";
        const title = $2("a.mb-3.text-white.text-lg.font-bold", obj).text().trim() ?? "";
        const image = $2(".w-24.h-24.object-cover.rounded-lg", obj).attr("src") ?? "";
        if (!id) continue;
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      for (const obj of $2(".flex.flex-col.items-center.justify-center picture img").toArray()) {
        const page = $2(obj).attr("src") ?? "";
        if (!page) {
    ...
    parseChapters($2) {
      const chapters = [];
      let lastNumber = null;
      const arrChapters = $2(".block.border.border-border.bg-card.mb-3.p-3.rounded").toArray();
      for (const obj of arrChapters) {
        const url = $2(obj).attr("href") ?? "";
        const name = $2(".text-lg.font-bold:not(.flex)", obj).text().trim() ?? "No Chpater Name";
        if (!url) continue;
        ...
        if (match && !isNaN(Number(match[1]))) {
          ...
          if (lastNumber === null) {
            ...
            chapNum = Number((lastNumber + 1e-3).toFixed(3));
          }
        }
        lastNumber = chapNum;
    ...
    parseMangaDetails($2, mangaId) {
      const descElement = $2(".order-1.bg-card.border.border-border.rounded.py-3");
      const title = $2(".my-3.font-bold.text-3xl", descElement).first().text().trim() ?? "";
      const image = $2(".flex.items-center.justify-center img", descElement).attr("src") ?? "https://paperback.moe/icons/logo-alt.svg";
      let desc = $2(".leading-6.my-3", descElement).text().trim() ?? "";
  ...
  var TCBScans = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      return this.parser.parseHomeSections($2, sectionCallback);
    }
  ...
  return __toCommonJS(TCBScans_exports);
})();
