...
class AEManga {
    ...
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            promises.push(this.requestManager.schedule(section.request, 1).then(response => {
                ...
                section.section.items = (0, AEMangaParser_1.parseSearch)($, AEMANGA_DOMAIN);
                sectionCallback(section.section);
            }));
        }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        const manga = (0, AEMangaParser_1.parseSearch)($, AEMANGA_DOMAIN);
...
},{"./AEMangaParser":71,"@paperback/types":61}],71:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const parseMangaDetails = ($, mangaId) => {
    ...
    const titles = [(0, entities_1.decodeHTML)($('h1.EnglishName', contentSection).text().trim().replace('(', '').replace(')', ''))];
    ...
    const description = (0, entities_1.decodeHTML)($('div.manga-details-extended h4', contentSection).eq(4).text().trim() ?? '');
    const arrayTags = [];
