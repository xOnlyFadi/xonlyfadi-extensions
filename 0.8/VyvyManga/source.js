...
var _Sources = (() => {
  ...
  // src/VyvyManga/VyvyManga.ts
  var VyvyManga_exports = {};
  __export(VyvyManga_exports, {
    VyvyManga: () => VyvyManga,
    VyvyMangaInfo: () => VyvyMangaInfo
  });
  ...
  // src/VyvyManga/VyvyMangaParser.ts
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push(decodeHTML2($2(".col-md-7 .title").text().trim() ?? ""));
    const altTitles = $2(".col-md-7 .title + p:not(.text-info)").text().trim().split("/");
    for (const title of altTitles) {
      titles.push(decodeHTML2(title));
    }
    const description = decodeHTML2(
      $2(".summary .content").text().trim() ?? ""
    );
    const authorElement = $2(".pre-title:contains(Author)").next("a");
    const author = authorElement.length ? authorElement.children().map((_, e) => {
      return $2(e).text().trim();
    }).toArray().join(", ") : "";
    const artistElement = $2(".pre-title:contains(Author)").next("a");
    const artist = artistElement.length ? artistElement.children().map((_, e) => {
      return $2(e).text().trim();
    }).toArray().join(", ") : "";
    const arrayTags = [];
    for (const tag of $2(".pre-title:contains(Genres)").siblings("a").toArray()) {
      const label = $2(tag).text().trim();
      const id = $2(tag).attr("href")?.split("/genre/").pop() ?? "";
    ...
    const status = $2(".pre-title:contains(Status) ~ span:not(.space)").text().trim();
    const image = $2(".img-manga img").attr("src") ?? "";
  ...
  var parseChapterList = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2(".list-group > a").toArray()) {
      const title = $2("span", chapter).text().trim();
      const chapterAttr = chapter.attribs;
      const chapterId = chapterAttr.id ?? "";
      if (!chapterId) continue;
      const chapNumRegex = chapterAttr.id?.match(/(\d+)(?:[-.]\d+)?/);
      let chapNum = chapNumRegex?.[1] ? Number(chapNumRegex[1].replace("-", ".")) : 0;
      if (isNaN(chapNum)) chapNum = 0;
      const date = parseDate($2("p", chapter).text().trim());
      chapters.push({
        id: chapterId,
        name: title,
        langCode: "ENG",
  ...
  var parseChapterURL = ($2, chapterId) => {
    let chapterUrl = "";
    for (const chapter of $2(".list-group > a").toArray()) {
      const chapterAttr = chapter.attribs;
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const img of $2("img.d-block").toArray()) {
      const page = $2(img).attr("data-src") ?? "";
      if (!page) continue;
      pages.push(page);
    }
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2(".weekly-book .slick-detail > a").toArray()) {
      const image = $2(".img-manga > img", manga).attr("src") ?? "";
      const title = $2(".title", manga).text().trim() ?? "";
      const subtitle = $2(".alternative-title", manga).text().trim() ?? "";
      const id = $2(manga).attr("href")?.split("/manga/").pop() ?? "";
    ...
    const latestSectionElem = $2('.home-title:contains("Latest Update")').closest(".latest-update");
    for (const manga of latestSectionElem.find(".book-list .col-4 > .comic-item").toArray()) {
      const image = $2(".comic-image > img", manga).attr("data-src") ?? "";
      const title = $2(".comic-title", manga).text().trim() ?? "";
      const subtitle = $2(".tray-item", manga).text().trim() ?? "";
      const id = $2("a", manga).attr("href")?.split("/manga/").pop() ?? "";
      if (!id || !title) continue;
    ...
    const newSectionElem = $2('.home-title:contains("New Release")').closest(".latest-update");
  ...
  var parseViewMore = ($2) => {
    const manga = [];
    const collectedIds = [];
    for (const obj of $2(".book-list .col-4 > .comic-item").toArray()) {
      const image = $2(".comic-image > img", obj).attr("data-src") ?? "";
      const title = $2(".comic-title", obj).text().trim() ?? "";
      const subtitle = $2(".tray-item", obj).text().trim() ?? "";
      const id = $2("a", obj).attr("href")?.split("/manga/").pop() ?? "";
  ...
  var parseSearch = ($2) => {
    const mangas = [];
    for (const obj of $2(".book-list .col-4 > .comic-item").toArray()) {
      const image = $2(".comic-image", obj).attr("data-background-image") ?? "";
      const title = $2(".comic-title", obj).text().trim() ?? "";
      const subtitle = $2(".tray-item", obj).text().trim() ?? "";
      const id = $2("a", obj).attr("href")?.split("/manga/").pop() ?? "";
      if (!id || !title) continue;
  ...
  var parseTags = ($2) => {
    const arrayTags = [];
    for (const tag of $2(".dropdown-genre ul li > a").toArray()) {
      const label = $2(tag).text().trim() ?? "";
      const id = $2(tag).attr("href")?.split("/genre/").pop() ?? "";
      if (!id || !label) continue;
      arrayTags.push({ id, label });
  ...
  var isLastPage = ($2) => {
    let isLast = false;
    const lastPageLink = $2("ul.pagination li.page-item a.page-link").last().attr("href");
    let lastPage = 1;
    if (lastPageLink) {
    ...
    const currentPage = Number($2("ul.pagination li.page-item.active").text().trim());
  ...
  // src/VyvyManga/VyvyManga.ts
  var VYVY_DOMAIN = "https://vymanga.net";
  var VyvyMangaInfo = {
    version: "2.0.2",
    name: "VyvyManga",
    icon: "icon.png",
  ...
  var VyvyManga = class _VyvyManga {
    constructor() {
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const chapterUrl = parseChapterURL($2, chapterId);
      request = App.createRequest({
        url: `${chapterUrl}`,
        method: "GET"
      });
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        request = App.createRequest({
          url: `${VYVY_DOMAIN}/genre/${query?.includedTags?.map((x) => x.id)[0]}`,
          param: `?page=${page}`,
          method: "GET"
        });
      }
...
Please go to the homepage of <${_VyvyManga.name}> and press the cloud icon.`
        );
      }
    }
  ...
  return __toCommonJS(VyvyManga_exports);
})();
