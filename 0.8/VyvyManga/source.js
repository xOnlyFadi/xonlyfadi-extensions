...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    ...
    const arrayTags = [];
    for (const tag of $(".pre-title:contains(Genres)").siblings("a").toArray()) {
    ...
    const status = $(".pre-title:contains(Status) ~ span:not(.space)").text().trim();
