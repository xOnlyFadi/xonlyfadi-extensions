  ...
  var Parser3 = class {
    ...
    parseHomeSection($2, baseUrl) {
      ...
      for (const obj of $2("div.element").toArray()) {
        ...
        const title = decodeHTML($2("h4.text-truncate", info).text().trim()) ?? decodeHTML($2("h4.text-truncate", info)?.attr("title")?.trim() ?? "") ?? "";
        const image = $2(obj).find("style").toString().substringAfterFirst("('").substringBeforeFirst("')") ?? "";
    ...
    parseMangaDetails($2, mangaId) {
      ...
      const title2 = decodeHTML($2("h2.element-subtitle").first().text().trim()) ?? "";
      const image = $2(".book-thumbnail").attr("src") ?? "https://paperback.moe/icons/logo-alt.svg";
      const desc = decodeHTML($2("p.element-description").text().trim()) ?? "";
      ...
      const status = decodeHTML($2("span.book-status").text().trim()) ?? "";
      const arrayTags = [];
