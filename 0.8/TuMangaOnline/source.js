...
var _Sources = (() => {
  ...
  var TuMangaOnline_exports = {};
  __export(TuMangaOnline_exports, {
    TuMangaOnline: () => TuMangaOnline,
    TuMangaOnlineInfo: () => TuMangaOnlineInfo
  });
  ...
  function isEntityInAttributeInvalidEnd3(nextCp) {
    return nextCp === CODE_POINTS.EQUALS_SIGN || isAsciiAlphaNumeric3(nextCp);
  }
  ...
  var Tokenizer = class {
    ...
    _matchNamedCharacterReference(cp) {
      ...
      for (let i = 0, current = decode_data_html_default2[0]; i >= 0; cp = this._consume()) {
        i = determineBranch2(decode_data_html_default2, current, i + 1, cp);
        if (i < 0)
          break;
        excess += 1;
        current = decode_data_html_default2[i];
        const masked = current & BinTrieFlags2.VALUE_LENGTH;
        if (masked) {
          const valueLength = (masked >> 14) - 1;
          if (cp !== CODE_POINTS.SEMICOLON && this._isCharacterReferenceInAttribute() && isEntityInAttributeInvalidEnd3(this.preprocessor.peek(1))) {
            result = [CODE_POINTS.AMPERSAND];
            i += valueLength;
          } else {
            result = valueLength === 0 ? [decode_data_html_default2[i] & ~BinTrieFlags2.VALUE_LENGTH] : valueLength === 1 ? [decode_data_html_default2[++i]] : [decode_data_html_default2[++i], decode_data_html_default2[++i]];
            excess = 0;
  ...
  function isEntityInAttributeInvalidEnd5(code) {
    ...
    stateNamedEntity(input, offset) {
      ...
      for (; offset < input.length; offset++, this.excess++) {
        ...
        if (this.treeIndex < 0) {
          ...
          (valueLength === 0 || // And there should be no invalid characters.
          isEntityInAttributeInvalidEnd5(char)) ? 0 : this.emitNotTerminatedNamedEntity();
        }
        current = decodeTree[this.treeIndex];
  ...
  // src/TuMangaOnline/TuMangaOnlineParser.ts
  var Parser3 = class {
    constructor() {
      this.idCleaner = (str, baseUrl) => {
    ...
    parseHomeSection($2, baseUrl) {
      const items = [];
      for (const obj of $2("div.element").toArray()) {
        const info = $2("div.element > a", obj);
        const id = this.idCleaner(info.attr("href")?.trim() ?? "", baseUrl) ?? "";
        const title = decodeHTML2($2("h4.text-truncate", info).text().trim()) ?? decodeHTML2($2("h4.text-truncate", info)?.attr("title")?.trim() ?? "") ?? "";
        const image = $2(obj).find("style").toString().split("('")[1]?.split("')")[0] ?? "";
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      for (const obj of $2("div.viewer-container img").toArray()) {
        const page = $2(obj).attr("data-src") ?? $2(obj).attr("src") ?? "";
        if (!page) {
    ...
    parseChapters($2, mangaId, baseUrl) {
      const chapters = [];
      const ChapterNumRegex = /capítulo ([\d.]+)?|capitulo ([\d.]+)?/i;
      if ($2("div.chapters").contents().length == 0) {
        for (const obj of $2("div.chapter-list-element > ul.list-group li.list-group-item").toArray()) {
          const id = this.idCleaner($2("div.row > .text-right > a", obj).attr("href") ?? "", baseUrl);
          const name = "One Shot";
          const scanlator = $2("div.col-md-6.text-truncate", obj).text().trim() ?? "";
          const date_upload = $2("span.badge.badge-primary.p-2", obj).first().text() ?? "";
        ...
        for (const obj of $2("div.chapters > ul.list-group li.p-0.list-group-item").toArray()) {
          const chapStyle = $2("a.btn-collapse", obj).text().trim();
          ...
          const name = $2("div.col-10.text-truncate", obj).text().trim();
          const scanelement = $2("ul.chapter-list > li", obj).toArray();
          for (const allchaps of scanelement) {
            const id = this.idCleaner($2("div.row > .text-right > a", allchaps).attr("href") ?? "", baseUrl);
            const scanlator = $2("div.col-md-6.text-truncate", allchaps).text().trim() ?? "";
            const date_upload = $2("span.badge.badge-primary.p-2", allchaps).first().text() ?? "";
            if (typeof id === "undefined") {
              throw new Error(`Could not parse out ID when getting chapters for ${mangaId}`);
            }
            chapters.push(App.createChapter({
    ...
    parseMangaDetails($2, mangaId) {
      const infotitle = $2("h1.element-title").first();
      infotitle.find("small").remove();
      const title = infotitle.text().trim() ?? "";
      const title2 = decodeHTML2($2("h2.element-subtitle").first().text().trim()) ?? "";
      const image = $2(".book-thumbnail").attr("src") ?? "https://paperback.moe/icons/logo-alt.svg";
      const desc = decodeHTML2($2("p.element-description").text().trim()) ?? "";
      const infoAuth = $2("h5.card-title");
      const author = decodeHTML2(infoAuth.first().text().trim().split(", ")[1] ?? "");
      const artist = decodeHTML2(infoAuth.last().text().trim().split(", ")[1] ?? "");
      const status = decodeHTML2($2("span.book-status").text().trim()) ?? "";
      const arrayTags = [];
      const genreregex = /genders.*?=(\d+)?/i;
      for (const obj of $2("a.py-2").toArray()) {
        const link = $2(obj)?.attr("href") ?? "";
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          desc: desc !== "" ? desc : "La fuente no proporciona ninguna decscripci\xF3n (TuMangaOnline)"
        })
      });
    }
    parseTags($2, isNSFW) {
      ...
      for (const tag of $2("#books-genders .col-auto .custom-control").toArray()) {
        const label = $2("label", tag).text().trim();
        const id = $2("input", tag).attr("value") ?? "0";
      ...
      return [
        App.createTagSection({ id: "0", label: "G\xE9neros", tags: arrayTags.map((x) => App.createTag(x)) }),
        ...
        App.createTagSection({ id: "5", label: "Filtrar por estado de traducci\xF3n", tags: arrayTags4.map((x) => App.createTag(x)) }),
        App.createTagSection({ id: "6", label: "Filtrar por demograf\xEDa", tags: arrayTags5.map((x) => App.createTag(x)) }),
  ...
  // src/TuMangaOnline/TuMangaOnlineSettings.ts
  var getNSFW = async (stateManager) => {
    return await stateManager.retrieve("nsfw") ?? false;
  };
  var contentSettings = (stateManager) => {
    return App.createDUINavigationButton({
      id: "content_settings",
      label: "Configuraci\xF3n del contenido",
      form: App.createDUIForm({
        sections: async () => [
          App.createDUISection({
            id: "content",
            footer: "Activa el NSFW en la aplicaci\xF3n",
            isHidden: false,
            rows: async () => [
              App.createDUISwitch({
                id: "nsfw",
  ...
  var TuMangaOnline = class _TuMangaOnline {
    constructor() {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const sections = [
        {
          ...
          section: App.createHomeSection({
            id: "1",
            title: "\xDAltimas actualizaciones",
      ...
      for (const section of sections) {
        ...
        promises.push(this.requestManager.schedule(section.request, 1).then((response) => {
          ...
          section.section.items = this.parser.parseHomeSection($2, TUMANGA_DOMAIN);
          sectionCallback(section.section);
        }));
      }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const $2 = load(response.data);
      const manga = this.parser.parseHomeSection($2, TUMANGA_DOMAIN);
    ...
    async getSearchTags() {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      return this.parser.parseTags($2, getNSFWOption);
    }
    ...
    async getChapters(mangaId) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      return this.parser.parseChapters($2, mangaId, TUMANGA_DOMAIN);
    }
    ...
    async getChapterURL(chapterId, mangaId) {
      ...
      this.CloudFlareError(data2.status);
      const $2 = load(data2.data);
      const button = $2(".flex-row button.btn-social").attr("onclick")?.match(/copyToClipboard\(['"`](.*)['"`]\)/i);
      let url;
      if (button?.[1]) {
    ...
    async getSearchResults(query, metadata) {
      ...
      const data2 = await this.requestManager.schedule(request, 2);
      this.CloudFlareError(data2.status);
      const $2 = load(data2.data);
      const manga = this.parser.parseHomeSection($2, TUMANGA_DOMAIN);
      metadata = this.parser.NextPage($2) ? { page: page + 1 } : void 0;
...
Please go to Settings > Sources > ${_TuMangaOnline.name} and press Cloudflare Bypass`);
      }
    }
  };
  return __toCommonJS(TuMangaOnline_exports);
})();
