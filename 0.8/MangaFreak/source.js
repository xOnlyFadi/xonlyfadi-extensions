...
var _Sources = (() => {
  ...
  var MangaFreak_exports = {};
  __export(MangaFreak_exports, {
    MangaFreak: () => MangaFreak,
    MangaFreakInfo: () => MangaFreakInfo
  });
  ...
  // src/MangaFreak/MangaFreakParser.ts
  ...
  var Parser3 = class {
    ...
    async parseHomeSections($2, sectionCallback, cdnUrl) {
      const top5Section = App.createHomeSection({ id: "top5", title: "Top 5", containsMoreItems: false, type: import_types2.HomeSectionType.featured });
      const popularSection = App.createHomeSection({ id: "popular", title: "Popular", containsMoreItems: true, type: import_types2.HomeSectionType.singleRowNormal });
      const TodayMangaSection = App.createHomeSection({ id: "today_manga", title: "Today's Manga", containsMoreItems: true, type: import_types2.HomeSectionType.singleRowNormal });
      const YesterdayMangaSection = App.createHomeSection({ id: "yesterday_manga", title: "Yesterday's Manga", containsMoreItems: false, type: import_types2.HomeSectionType.singleRowNormal });
      const OlderMangaSection = App.createHomeSection({ id: "older_manga", title: "Older Manga", containsMoreItems: false, type: import_types2.HomeSectionType.singleRowNormal });
      const Top5 = [];
      const Popular = [];
      const TodayManga = [];
      const YesterdayManga = [];
      const OlderManga = [];
      for (const obj of $2("li", $2(".slide_box .rslides")).toArray()) {
        const id = $2("a", obj).attr("href")?.split("/").pop() ?? "";
        const title = $2("div", obj).text().trim() ?? "";
      ...
      for (const obj of $2(".featured_item_info", $2(".box .featured_list div")).toArray()) {
        const id = $2("a", obj).first().attr("href")?.split("/").pop() ?? "";
        const title = $2("a", obj).first().text().trim() ?? "";
        const image = `${cdnUrl}/manga_images/${id.toLowerCase()}.jpg`;
        const subtitle = $2("a#chapter", obj).text().trim();
        if (!id) continue;
  ...
  var MangaFreak = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const manga = this.parser.parseSearchResults($2, MangaFreak_CDN, UsesDeatils);
      metadata = this.parser.NextPage($2) ? { page: page + 1 } : void 0;
      return App.createPagedResults({
        results: manga,
        metadata
...
Please go to Settings > Sources > ${MangaFreakInfo.name} and press Cloudflare Bypass`);
  ...
  return __toCommonJS(MangaFreak_exports);
})();
