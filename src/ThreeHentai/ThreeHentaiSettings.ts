...
export const getLanguages = async (stateManager: SourceStateManager): Promise<string[]> => {
    return (await stateManager.retrieve('languages') as string[]) ?? THLanguages.getDefault()
}
...
export const getSortOrders = async (stateManager: SourceStateManager): Promise<string[]> => {
    return (await stateManager.retrieve('sort_order') as string[]) ?? THSortOrders.getDefault()
}
                        ...
                        rows: () => {
                            return Promise.all([
                                ...
                                createSelect({
                                    id: 'languages',
                                    label: 'Languages',
                                    options: THLanguages.getTHCodeList(),
                                    displayLabel: option => THLanguages.getName(option),
                                    value: values[0],
                                ...
                                createSelect({
                                    ...
                                    options: THSortOrders.getTHCodeList(),
                                    displayLabel: option => THSortOrders.getName(option),
                                    value: values[1],
                                    allowsMultiselect: false,
                                    minimumOptionCount: 1,
                                }),
