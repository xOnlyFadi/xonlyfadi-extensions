...
class Tsumino extends paperback_extensions_common_1.Source {
    constructor() {
        super(...arguments);
    ...
    getMangaShareUrl(mangaId) { return `${TSUMINO_DOMAIN}/entry/${mangaId}`; }
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            promises.push(this.requestManager.schedule(section.request, 1).then(response => {
                ...
                section.section.items = (0, TsuminoParser_1.parseSearch)(data);
                sectionCallback(section.section);
            }));
        }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const manga = (0, TsuminoParser_1.parseSearch)(data);
    ...
    async getMangaDetails(mangaId) {
        ...
        return (0, TsuminoParser_1.parseMangaDetails)($, mangaId);
    }
    async getChapters(mangaId) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        return (0, TsuminoParser_1.parseChapters)($, mangaId);
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        return (0, TsuminoParser_1.parseChapterDetails)($, mangaId, chapterId);
    }
...
const parseMangaDetails = ($, mangaId) => {
    ...
    for (const obj of $('#Artist a').toArray()) {
        ...
        if (!author)
            continue;
        artists.push((0, entities_1.decodeHTML)(author));
    }
    const groups = [];
    for (const obj of $('#Group a').toArray()) {
        ...
        if (!group)
            continue;
        groups.push((0, entities_1.decodeHTML)(group));
    }
    const arrayTags = [];
    for (const tag of $('#Tag a').toArray()) {
        ...
        if (!id || !label)
            continue;
        arrayTags.push({
            id: `genres.${(0, entities_1.decodeHTML)(id)}`,
            label
        });
    }
...
const parseChapters = ($, mangaId) => {
    ...
    if (date && date[0] && date[1] && date[2]) {
        time = new Date(Number(date[0]), Number(exports.months[date[1]]), Number(date[2]));
    }
    if (collection.contents().length !== 0) {
        for (const chapter of $('.book-collection-table a').toArray()) {
            ...
            chapters.push(createChapter({
                id: id,
                mangaId,
                name: (0, entities_1.decodeHTML)(name),
